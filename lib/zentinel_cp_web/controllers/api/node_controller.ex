defmodule ZentinelCpWeb.Api.NodeController do
  @moduledoc """
  API controller for node-facing endpoints.
  These endpoints are called by Zentinel nodes, not operators.
  """
  use ZentinelCpWeb, :controller

  alias ZentinelCp.{Analytics, Audit, Auth, Nodes, Projects, Rollouts}
  ...
  def latest_bundle(conn, _params) do
    ...
    case node.staged_bundle_id do
      ...
      staged_id when staged_id != node.active_bundle_id ->
        bundle = ZentinelCp.Bundles.get_bundle(staged_id)

        if bundle && bundle.status == "compiled" do
          download_url =
            case ZentinelCp.Bundles.Storage.presigned_url(bundle.storage_key) do
              {:ok, url} -> url
              _ -> nil
            end
