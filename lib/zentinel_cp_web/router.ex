defmodule ZentinelCpWeb.Router do
  use ZentinelCpWeb, :router

  import ZentinelCpWeb.Plugs.Auth,
    only: [
  ...
  pipeline :browser do
    ...
    plug :put_root_layout, html: {ZentinelCpWeb.Layouts, :root}
  ...
  pipeline :node_auth do
    plug ZentinelCpWeb.Plugs.NodeAuth
  end

  pipeline :api_auth do
    plug ZentinelCpWeb.Plugs.ApiAuth
    plug ZentinelCpWeb.Plugs.RateLimit, scope: "default"
  end

  pipeline :graphql do
    plug ZentinelCpWeb.GraphQL.Context
  end

  # Scope-specific pipelines
  pipeline :require_nodes_read do
    plug ZentinelCpWeb.Plugs.RequireScope, scope: "nodes:read"
  end

  pipeline :require_nodes_write do
    plug ZentinelCpWeb.Plugs.RequireScope, scope: "nodes:write"
  end

  pipeline :require_bundles_read do
    plug ZentinelCpWeb.Plugs.RequireScope, scope: "bundles:read"
  end

  pipeline :require_bundles_write do
    plug ZentinelCpWeb.Plugs.RequireScope, scope: "bundles:write"
  end

  pipeline :require_rollouts_read do
    plug ZentinelCpWeb.Plugs.RequireScope, scope: "rollouts:read"
  end

  pipeline :require_rollouts_write do
    plug ZentinelCpWeb.Plugs.RequireScope, scope: "rollouts:write"
  end

  pipeline :require_services_read do
    plug ZentinelCpWeb.Plugs.RequireScope, scope: "services:read"
  end

  pipeline :require_services_write do
    plug ZentinelCpWeb.Plugs.RequireScope, scope: "services:write"
  end

  pipeline :require_api_keys_admin do
    plug ZentinelCpWeb.Plugs.RequireScope, scope: "api_keys:admin"
  end

  # Health checks (no auth, no session)
  scope "/", ZentinelCpWeb do
    pipe_through :api
  ...
  scope "/.well-known/acme-challenge", ZentinelCpWeb.Api do
    pipe_through :api
  ...
  scope "/" do
    ...
    get "/metrics", ZentinelCpWeb.PromExPlug, prom_ex_module: ZentinelCp.PromEx
  end
  ...
  scope "/auth", ZentinelCpWeb.Auth do
    pipe_through [:browser]
  ...
  scope "/portal/:project_slug", ZentinelCpWeb.PortalLive do
    pipe_through [:browser, ZentinelCpWeb.Plugs.PortalAccess]
  ...
  scope "/api/v1/webhooks", ZentinelCpWeb.Api do
    pipe_through :api
  ...
  # Node-facing API (called by Zentinel nodes)
  scope "/api/v1", ZentinelCpWeb.Api do
    pipe_through :api
  ...
  scope "/api/v1/nodes", ZentinelCpWeb.Api do
  ...
  scope "/api/v1" do
    ...
    forward "/graphql", Absinthe.Plug,
      schema: ZentinelCpWeb.GraphQL.Schema,
      json_codec: Jason
  end

  # Enable LiveDashboard and Swoosh mailbox preview in development
  if Application.compile_env(:zentinel_cp, :dev_routes) do
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through :browser

      live_dashboard "/dashboard", metrics: ZentinelCpWeb.Telemetry
    ...
    scope "/dev" do
      forward "/graphiql", Absinthe.Plug.GraphiQL,
        schema: ZentinelCpWeb.GraphQL.Schema,
        socket: ZentinelCpWeb.GraphQL.Socket,
        json_codec: Jason
    end
  end
end
