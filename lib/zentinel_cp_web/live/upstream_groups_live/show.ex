defmodule ZentinelCpWeb.UpstreamGroupsLive.Show do
    ...
    with project when not is_nil(project) <- Projects.get_project_by_slug(slug),
      ...
      if connected?(socket) do
        if discovery_source do
          Phoenix.PubSub.subscribe(ZentinelCp.PubSub, "discovery:#{discovery_source.id}")
        end

        Phoenix.PubSub.subscribe(ZentinelCp.PubSub, "circuit_breaker:#{group.id}")
      end
  ...
  def handle_event("create_discovery", params, socket) do
    ...
    case Services.create_discovery_source(attrs) do
      {:ok, source} ->
        if connected?(socket) do
          Phoenix.PubSub.subscribe(ZentinelCp.PubSub, "discovery:#{source.id}")
        end
  ...
  def handle_event("delete_discovery", _, socket) do
    ...
    if connected?(socket) do
      Phoenix.PubSub.unsubscribe(ZentinelCp.PubSub, "discovery:#{source.id}")
    end
