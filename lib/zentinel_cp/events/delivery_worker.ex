defmodule ZentinelCp.Events.DeliveryWorker do
  @moduledoc """
  ...
  alias ZentinelCp.Events.{DeliveryAttempt, Channel}
  alias ZentinelCp.Events.Adapters
  ...
  defp deliver_to_channel(event, %Channel{type: "email", config: config}) do
    ...
    to = config["to"]
    from = config["from"] || "noreply@zentinel-cp.local"
