defmodule ZentinelCp.Rollouts do
  @moduledoc """
  ...
  alias ZentinelCp.{Bundles, Nodes, Orgs, Projects}
  ...
  def active_freeze_window(project_id, environment_id) do
    ...
    query =
      from(w in ZentinelCp.Rollouts.FreezeWindow,
        where:
  ...
  defp log_audit(rollout, action) do
    unless Application.get_env(:zentinel_cp, :env) == :test do
      ZentinelCp.Audit.log_system_action(action, "rollout", rollout.id,
  ...
  defp complete_step(rollout, step) do
    ...
    ZentinelCp.Nodes.set_expected_bundle_for_nodes(step.node_ids, rollout.bundle_id)
  ...
  defp broadcast_rollout_update(rollout) do
    ...
    Absinthe.Subscription.publish(ZentinelCpWeb.Endpoint, rollout, rollout_progress: rollout.id)
  end
