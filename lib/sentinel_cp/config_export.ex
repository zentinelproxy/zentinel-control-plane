defmodule SentinelCp.ConfigExport do
  @moduledoc """
  ...
  def export(project_id) do
    project = Repo.get!(SentinelCp.Projects.Project, project_id)
  ...
  defp export_environments(project_id) do
    from(e in SentinelCp.Projects.Environment,
  ...
  defp export_upstream_groups(project_id) do
    from(g in SentinelCp.Services.UpstreamGroup,
  ...
  defp export_certificates(project_id) do
    from(c in SentinelCp.Services.Certificate,
  ...
  defp export_auth_policies(project_id) do
    from(a in SentinelCp.Services.AuthPolicy,
  ...
  defp export_middlewares(project_id) do
    from(m in SentinelCp.Services.Middleware,
  ...
  defp export_services(project_id) do
    from(s in SentinelCp.Services.Service,
  ...
  defp import_environments(project_id, envs, summary) do
    Enum.reduce(envs, summary, fn env_data, acc ->
      name = env_data["name"]
      existing = Repo.get_by(SentinelCp.Projects.Environment, project_id: project_id, name: name)

      if existing do
        %{acc | skipped: acc.skipped + 1}
      else
        case %SentinelCp.Projects.Environment{}
  ...
  defp import_upstream_groups(project_id, groups, summary) do
    Enum.reduce(groups, summary, fn group_data, acc ->
      ...
      if existing do
        ...
        case SentinelCp.Services.create_upstream_group(%{
               project_id: project_id,
               name: name,
