defmodule SentinelCpWeb.Api.PluginControllerTest do
  ...
  describe "service plugin chain" do
    ...
    test "reorder plugins", %{conn: conn, project: project} do
      ...
      {:ok, sp1} =
        SentinelCp.Plugins.attach_plugin(%{service_id: service.id, plugin_id: p1.id, position: 0})

      {:ok, sp2} =
        SentinelCp.Plugins.attach_plugin(%{service_id: service.id, plugin_id: p2.id, position: 1})
