defmodule SentinelCp.Auth.SsoTest do
    ...
    test "respects group → role mapping", %{org: org} do
      {:ok, provider} =
        Sso.create_oidc_provider(%{
          ...
          discovery_url: "https://okta.example.com/.well-known/openid-configuration",
          default_role: "reader",
          group_mapping: %{"sentinel-admins" => "admin", "sentinel-ops" => "operator"}
        })

      admin_info = %{sub: "admin1", email: "admin@example.com", groups: ["sentinel-admins"]}
      {:ok, admin} = Sso.process_sso_login(:oidc, provider, admin_info)
      assert admin.role == "admin"

      ops_info = %{sub: "ops1", email: "ops@example.com", groups: ["sentinel-ops"]}
